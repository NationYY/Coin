...
void COkexHttpAPI::API_SwapFuturesTransferToZiJin(bool bSync, std::string& strCoinType, std::string& standardCurrency, std::string amount, SHttpResponse* pResInfo)
{
	SHttpReqInfo info;
	...
	info.mapParams["from"] = SHttpParam(eHttpParamType_String, "9");
	info.mapParams["to"] = SHttpParam(eHttpParamType_String, "6");
	std::string instrument_id = strCoinType + "-" + standardCurrency;
	info.mapParams["instrument_id"] = SHttpParam(eHttpParamType_String, instrument_id.c_str());
	if(bSync)
		RequestAsync(info);
	else
		Request(info, pResInfo);
